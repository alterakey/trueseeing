...
class CryptoStaticKeyDetector(Detector):
  ...
  def do_detect_case1(self):
    ...
    for cl in self.context.store().query().invocations(InvocationPattern('invoke-', 'Ljavax?.*/(SecretKey|(Iv|GCM)Parameter|(PKCS8|X509)EncodedKey)Spec|Ljavax?.*/MessageDigest;->(update|digest)')):
      try:
  ...
  def do_detect_case2(self):
    ...
    for cl in self.context.store().query().invocations(InvocationPattern('const-string', pat)):
      val = k.p[1].v
...
class CryptoEcbDetector(Detector):
  ...
  def do_detect(self):
    for cl in self.context.store().query().invocations(InvocationPattern('invoke-static', 'Ljavax/crypto/Cipher;->getInstance\(Ljava/lang/String;.*?\)')):
      try:
