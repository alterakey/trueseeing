...
def detect_urllike(context):
  ...
  def analyzed(x):
    if '://' in x:
      ...
      elif re_tlds.search(components[-1]):
  ...
  with open(pkg_resources.resource_filename(__name__, os.path.join('libs', 'tlds.txt')), 'r') as f:
    re_tlds = re.compile('^(?:%s)$' % '|'.join(re.escape(l.strip()) for l in f if l and not l.startswith('#')), flags=re.IGNORECASE)
  
  marks = []
  for cl in context.analyzed_classes():
    for k in OpMatcher(cl.ops, InvocationPattern('const-string', '.')).matching():
      for match in analyzed(k.p[1].v):
        for v in match['value']:
          marks.append(dict(name=context.class_name_of_dalvik_class_type(cl.qualified_name()), method=k.method_, op=k, target_val=v, target_type=match['type_']))
  for name, val in context.string_resources():
    for match in analyzed(val):
      for v in match['value']:
