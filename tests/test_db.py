...
class P:
  ...
  def parsed(s):
    ...
    if q:
      class_sig = P.parsed_as_op(q.popleft())
      while q:
        t = P.parsed_as_op(q[0])
        if t.v in ('super', 'source'):
          class_sig.p.append(t)
          q.popleft()
        else:
          break
      yield class_sig
      yield from P.parsed_whole(q)
      ...
      if l:
        ...
        if x.t != 'directive':
          ...
          if x.v != 'annotation':
            ...
            if x.p is None:
              x.p = []
            x.p.append(P.parsed_as_annotation_content(q))
            yield x
